...
- [ ] **🎯 연습 패턴 시퀀스 (v1.2.0 목표)**: 훈련용 패턴을 다중 Phase로 구성하고 애니메이션 재생
  - 익명 번호 선수 (1, 2, 3...) 배치
  - Phase 1→2→3 시퀀스 구성
  - 패턴 라이브러리 및 카테고리 관리
  - 재생/일시정지/속도 조절
...
### v1.2.0: 연습 패턴 시스템 (배포 후 2-3개월) 🎯 NEW

**핵심 신규 기능: 연습 패턴 시퀀스**
- [ ] **패턴 편집기**: 익명 번호 선수(1,2,3...) 자유 배치
- [ ] **다중 Phase 관리**: Phase 1→2→3 시퀀스 구성
- [ ] **패턴 라이브러리**: 저장/불러오기, 카테고리 분류
- [ ] **기본 재생 기능**: Phase 순차 전환
- [ ] **5가지 기본 템플릿**: 일반적인 연습 패턴 프리셋

**사용 시나리오 예시:**
- "2:1 패스로 수비 따돌리기" (3명, 3 Phase)
- "리턴패스 후 침투" (2명, 2 Phase)
- "오버랩 연계" (3명, 4 Phase)

**예상 개발 기간:** 2-3주

### v1.3.0: 패턴 애니메이션 & 고급 기능 (배포 후 4-5개월)

- [ ] **패턴 재생 강화**: 부드러운 Phase 전환 애니메이션
- [ ] **재생 컨트롤**: 재생/일시정지/속도 조절/이전/다음
- [ ] **패턴 공유**: Export/Import JSON
